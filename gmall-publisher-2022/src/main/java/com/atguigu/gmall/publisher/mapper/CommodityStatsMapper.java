...
public interface CommodityStatsMapper {
     ...
     */
    @Select("select trademark_name,sum(order_amount) order_amount from dws_trade_sku_order_window where toYYYYMMDD(stt)=#{date} group by trademark_name order by order_amount desc limit #{limit}")
